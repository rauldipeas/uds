...
add_ppa() {
    if [ "$(grep '^ID=' /etc/os-release | cut -d '=' -f2)" == ubuntu ]; then
        ...
        wget -qO- "https://keyserver.ubuntu.com/pks/lookup?op=get&search=$PPA_KEY" | sudo gpg --dearmor --yes -o /etc/apt/trusted.gpg.d/"$PPA_NAME".gpg
...
gei() {
    ...
    EXT_INFO="$(curl -sSL "https://extensions.gnome.org/extension-info/?pk=$ID&shell_version=$SHELL_VER")"
