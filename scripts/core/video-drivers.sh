...
TARGET="$(curl -sL https://api.github.com/repos/ilya-zlobintsev/LACT/releases | grep browser_download_url | grep download | grep 2404.deb | head -n1 | cut -d '"' -f4)"
...
if printf "%s" "$gpu_info" | grep -q NVIDIA >/dev/null; then
	...
	cd /tmp
	rm -f /tmp/*.deb
	wget -q --show-progress "$(curl -sL https://api.github.com/repos/Umio-Yasuno/amdgpu_top/releases|grep browser_download_url|grep amd64.deb|head -n1|cut -d '"' -f4)"
