#!/bin/bash
set -e
PROTON_VPN_VERSION="$(curl -sSL https://repo.protonvpn.com/debian/dists/stable/main/binary-all/ | grep -oP 'protonvpn-stable-release_\K[0-9.]+(?=_all\.deb)' | sort -V | tail -n1)"
