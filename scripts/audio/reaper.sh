...
wget -q --show-progress -O "$HOME"/.config/REAPER/LangPack/pt-BR.ReaperLangPack https://stash.reaper.fm"$(curl -sL https://stash.reaper.fm/tag/Language-Packs | grep pt-BR | head -n1 | cut -d '"' -f2 | sed 's/\/v//g')"
wget -q --show-progress -O "$HOME"/.config/REAPER/UserPlugins/reaper_reapack-x86_64.so "$(curl -sL https://api.github.com/repos/cfillion/reapack/releases | grep browser_download_url | grep download/v | grep x86_64.so | head -n1 | cut -d '"' -f4)"
cd /tmp
rm -f /tmp/*.tar.xz
wget -q --show-progress https://sws-extension.org/download/pre-release/"$(curl -sL http://sws-extension.org/download/pre-release/ | grep Linux-x86_64 | head -n1 | cut -d '"' -f4)"
