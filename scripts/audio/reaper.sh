...
cd /tmp
rm -f /tmp/*.tar.xz
wget -q --show-progress https://sws-extension.org/download/pre-release/"$(curl -sSL http://sws-extension.org/download/pre-release/ | grep Linux-x86_64 | head -n1 | cut -d '"' -f4)"
