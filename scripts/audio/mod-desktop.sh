...
cd /tmp/mod
rm -f /tmp/mod/*.xz
#wget -q --show-progress "$(curl -sSL https://api.github.com/repos/mod-audio/mod-desktop/releases|grep browser_download_url|grep download|grep linux-x86_64|head -n1|cut -d '"' -f4)"
wget -q --show-progress "$(curl -sSL https://mod.audio/desktop | grep tar.xz | cut -d '"' -f4)"
