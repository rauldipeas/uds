...
Type=Application
Name=yabridge sync
Exec=gnome-terminal --title="yabridge sync" --geometry=80x24 -- bash -c "yabridgectl sync --prune --verbose; printf 'Pressione qualquer tecla para fechar...'; read -n1"
Icon=blueman-plugin
Terminal=false
...
Type=Application
Name=yabridge sync
Exec=xterm -T 'yabridge sync' -fa 'Ubuntu Mono' -fs 11 -bg "#300a25" -fg white -e bash -c "yabridgectl sync --prune --verbose; printf 'Pressione qualquer tecla para fechar...'; read -n1"
Icon=blueman-plugin
Terminal=false
Categories=System;Utility;
StartupNotify=true
